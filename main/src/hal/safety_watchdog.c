...
void safety_watchdog_init(safety_watchdog_t *watchdog)
{
    ...
    gpio_config_t cfg = {
        .pin_bit_mask = (1ULL << watchdog->diag_left_io) | (1ULL << watchdog->diag_right_io),
    ...
    ESP_LOGI(TAG, "Safety watchdog initialized on DIAG pins %d and %d", watchdog->diag_left_io, watchdog->diag_right_io);
}
